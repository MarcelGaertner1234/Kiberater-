    ...
    <NotionCard gradient className="relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          <div>
            <h1 className={styles.text('h2', 'mb-2')}>Willkommen zurück, {user.name}! 👋</h1>
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {statsData.map((stat, index) => (
        <NotionCard key={index} className="text-center">
          <div
            className={`inline-flex items-center justify-center w-12 h-12 rounded-full bg-${stat.color}/10 text-${stat.color} mb-3`}
          >
            {stat.icon}
          </div>
          ...
          >
            <TrendingUp
              className={`w-3 h-3 mr-1 ${stat.change.type === 'decrease' ? 'rotate-180' : ''}`}
            />
            {stat.change.type === 'increase' ? '+' : '-'}
            {stat.change.value} diese Woche
          </div>
        </NotionCard>
      ))}
    </div>
    ...
    <NotionCard>
      ...
      <div className="space-y-4">
        {recentProjects.map((project) => (
          <div
            key={project.id}
            className="p-4 border border-notion-border dark:border-notion-dark-border rounded-lg hover:bg-notion-bg-hover dark:hover:bg-notion-dark-bg-hover transition-colors cursor-pointer"
          >
        ...
        {actions.map((action, index) => (
          ...
          >
            <div className="flex items-center">
              <div
                className={`w-10 h-10 rounded-lg bg-${action.color}/10 text-${action.color} flex items-center justify-center mr-3 group-hover:scale-110 transition-transform`}
              >
                {action.icon}
              </div>
              <div>
    ...
    <NotionCard>
      ...
      <div className="space-y-3">
        {upcomingTasks.map((task) => (
          <div
            key={task.id}
            className="flex items-start p-3 rounded-lg hover:bg-notion-bg-hover dark:hover:bg-notion-dark-bg-hover transition-colors cursor-pointer"
          >
            <div
              className={`w-8 h-8 rounded-lg bg-${getTaskColor(task.type)}/10 text-${getTaskColor(task.type)} flex items-center justify-center mr-3 mt-0.5`}
            >
